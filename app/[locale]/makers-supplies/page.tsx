...
export default async function MakersSuppliesPage({
    ...
    const { data: products } = await supabase
        .from('products')
        .select('*')
        .eq('category_slug', 'supplies');

    return (
        <main className="min-h-screen bg-[#FBFBF9] p-12 md:p-24">
            <header className="mb-20 text-center">
                <h1 className="font-serif text-5xl text-[#003D4D] italic">Makers Supplies</h1>
                <p className="mt-4 text-[10px] uppercase tracking-[0.5em] text-stone-400">Essentials for the creator</p>
            </header>
            ...
            <div className="max-w-[1800px] mx-auto grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                {products && products.length > 0 ? (
                    ...
                    <div className="col-span-full py-20 text-center border border-dashed border-stone-200">
                        <p className="font-serif italic text-stone-400">Inventory is currently being restocked...</p>
                    </div>
                )}
            </div>
        </main>
