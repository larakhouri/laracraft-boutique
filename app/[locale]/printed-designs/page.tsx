...
export default async function PrintedDesignsPage() {
    ...
    // 🟢 CLEAN FETCH: Fetching all designs without the buggy filters
    ...
    return (
        <main className="min-h-screen w-full bg-[#F8F6F1]">
            <ArtisanHero
                ...
                subtitle="Curated Artisan Creations • 2026 Edition"
                Icon={Scroll}
            />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-12">
                    {products?.map((item) => (
                        <Link key={item.id} href={`/product/${item.id}`} className="group block text-center">
                            ...
                            <p className="text-[10px] tracking-widest text-stone-400 uppercase mt-2">View Archive →</p>
                        </Link>
                    ))}
                </div>
            </div>
